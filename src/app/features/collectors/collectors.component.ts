...
export class CollectorsComponent {
  ...
  dataCollector = new MatTableDataSource<Collector>();
  collectorCol: string[] = ['name','address','phone','guarantorp','guarantors'];
  clienteParaImprimir!: ClienteCollector;
  showErrorModal = false;
  errorMessage = '';
  asignarDatos(respuesta: ClienteCollector): void {
    console.log('Asignar datos:', respuesta);
    const avalPrincipal = respuesta.avales?.[0];
    const avalSecundario = respuesta.avales?.[1];
    const collector: Collector = {
      ...
      guarantorp: avalPrincipal? `${avalPrincipal.nombre} ${avalPrincipal.apellidoPaterno} ${avalPrincipal.apellidoMaterno}`: '',
      guarantors: avalSecundario? `${avalSecundario.nombre} ${avalSecundario.apellidoPaterno} ${avalSecundario.apellidoMaterno}`: ''
    };
  ...
  imprimirCliente(): void {
    ...
    if (!this.clienteParaImprimir.credito) { 
      this.errorMessage = 'El cliente no tiene crédito.'; 
      this.showErrorModal = true; return; 
    }
    ...
    logo.onload = () => {
      ...
      const zonaInfo =
        `Zona: ${cliente.codigoZona}        Promotor: ${cliente.promotor}`;
      ...
      if (avales.length === 1) {
        ...
        let maxY = yAvales;
        for (let i = 0;i < Math.min(2, avales.length);i++) {
          const aval = avales[i];
          const x = columnaX[i];
          let yCol = yAvales;
          doc.setFontSize(11);
      ...
      // Control de cumplimiento de pago
      ...
      for (let fila = 0;fila < filasNecesarias;fila++) {
        ...
        pagosFila.forEach((pago, index) => {
          ...
          const fechaPago =
            new Date(pago.fechaEsperada).toLocaleDateString();
          doc.setFontSize(9);
          doc.text(fechaPago,x + 2,y);
        });
        y += lh;
