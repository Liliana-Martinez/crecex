...
export class PrintComponent {
  ...
  const renderContenido = (offsetY: number) => {
    ...
    doc.text(
      'FORMATO DE CREDITO FINANCIERA CRECEX',
      105,
      20 + offsetY,
      { align: 'center' }
    );
    ...
    doc.text(
      `HORARIO DE ENTREGA: ${(credito?.horarioEntrega ?? 'N/A').toUpperCase()}`,
      135,
      35 + offsetY
    );

    doc.text(
      `CODIGO:${credito.id || 'N/A'}`,
      135,
      45 + offsetY
    );

    doc.text(
      `SECCIÓN: ${zona.codigoZona || '-'}`,
      10,
      35 + offsetY
    );

    doc.text(
      `PROMOTOR: ${(zona.promotor || '-').toUpperCase()}`,
      90,
      45 + offsetY
    );
    ...
    imprimirPar(
      'TIPO DE CREDITO',
      (credito?.tipoCredito ?? 'N/A').toUpperCase(),
      false,
      'MONTO',
      `$${(credito?.monto ?? 0).toLocaleString('en-US')}`,
      true
    );

    imprimirPar(
      'ABONO SEMANAL ANTERIOR',
      `$${(abonoAnterior ?? 0).toLocaleString('en-US')}`,
      false,
      'SEMANAS RESTANTES',
      `${(semanasRestantes ?? 0).toLocaleString('en-US')}`,
      false
    );

    imprimirPar(
      'DEDUCCION DE SEMANAS',
      `$${(descuentoSemanas ?? 0).toLocaleString('en-US')}`,
      false,
      'ATRASOS',
      `$${(credito?.atrasos ?? 0).toLocaleString('en-US')}`,
      false
    );

    imprimirPar(
      'RECARGOS',
      `$${(credito?.recargos ?? 0).toLocaleString('en-US')}`,
      false,
      'DED. TOTALES',
      `$${(deduccionTotal ?? 0).toLocaleString('en-US')}`,
      false
    );

    imprimirPar(
      'CRÉDITO A ENTREGAR',
      `$${(credito?.efectivo ?? 0).toLocaleString('en-US')}`,
      true,
      'SEMANAS CREDITO',
      `${credito?.semanas ?? 0} semanas`,
      false
    );

    imprimirPar(
      'ABONO SEMANAL',
      `$${(credito?.abonoSemanal ?? 0).toLocaleString('en-US')}`,
      true,
      'FECHA PRIMER PAGO',
      fechaPrimerPago,
      true
  ...
  logo.onerror = () => {
    ...
    console.warn(
      'No se cargó logo, se genera sin imagen'
    );
